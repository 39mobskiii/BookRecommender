...
SELECT
    u.id AS IdUser,
    u.username AS [User],
    CONCAT(N'User', u.id) AS [User image name],
    b.id AS IdBook,
    b.title AS [Book],
    CONCAT(N'Book', b.id) AS [Book image name],
    r.user_rating AS [User rating],
    IIF(r.status = N'finished',
        N'Прочитана',
        N'Не дочитана / в планах') AS [Reading status]
FROM dbo.[User] AS u
   , dbo.Reads AS r
   , dbo.Book  AS b
WHERE MATCH(u-(r)->b);
...
SELECT
    u_from.id AS IdRecommender,
    u_from.username AS [Recommender],
    CONCAT(N'User', u_from.id) AS [Recommender image name],
    u_to.id AS IdReceiver,
    u_to.username AS [Receiver],
    CONCAT(N'User', u_to.id) AS [Receiver image name],
    rec.book_id AS [Recommended book ID],
    rec.recommend_date AS [Recommend date],
    IIF(rec.comment IS NOT NULL,
        N'С комментарием',
        N'Без комментария') AS [Has comment]
FROM dbo.[User] AS u_from
   , dbo.Recommends AS rec
   , dbo.[User]     AS u_to
WHERE MATCH(u_from-(rec)->u_to);
...
SELECT
    ...
    a.id AS IdAuthor,
    a.name AS [Author],
    CONCAT(N'Author', a.id) AS [Author image name],
    wb.role AS [Role],
    IIF(wb.contribution_pct = 100,
        N'Единственный автор',
        N'Соавтор / переводчик') AS [Contribution type]
FROM dbo.Book AS b
   , dbo.WrittenBy AS wb
   , dbo.Author    AS a
WHERE MATCH(b-(wb)->a);
...
SELECT
    ...
    g.id AS IdGenre,
    g.name AS [Genre],
    CONCAT(N'Genre', g.id) AS [Genre image name],
    bt.is_primary AS [Is primary],
    IIF(bt.is_primary = 1,
        N'Основной жанр',
        N'Второстепенный жанр') AS [Genre type],
    bt.tag AS [Tag]
FROM dbo.Book AS b
   , dbo.BelongsTo AS bt
   , dbo.Genre     AS g
